...
def gap_equation(t, delta):
    ...
    for n in tqdm(range(N + 1), desc="Matsubara frequencies"):
        ...
        temp += 1 / np.sqrt(1 + en**2) * np.pi**2 / 2  # bulk part of the gap equation, after the thetap integration
        ...
        # thetap integral
        F1_integrated = np.trapz(np.cos(thetapspan)[:, None] * F1_thetap, thetapspan, axis=0)
        F2_integrated = np.trapz(np.sin(thetapspan)[:, None] * F2_thetap, thetapspan, axis=0)
