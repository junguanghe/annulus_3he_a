...
def pwave_jsphsn_current_opposite(
    ...
    *,
    ...
    gap_mat_path : Path or str, optional
        Path to the gap profile ``gap.mat``. Defaults to the file in the current directory.
    ...
    if thetapspan is None:
        ...
        if thetap_values.size == 0:
            raise ValueError("'thetapspan' must contain at least one angle if provided.")

    prefactor = 16.0 * t / (delta0 * pi**2)
    cos_weights = np.cos(thetap_values)

    current = np.zeros_like(theta_values, dtype=float)
    gap_mat_path = Path(gap_mat_path) if gap_mat_path is not None else None
    ...
    for n in iterator:
        ...
        for idx_thetap, thetap in enumerate(thetap_values):
        ...
        if np.max(np.abs(updated_current - current)) < tolerance:
