...
def calc_chiral(n, R0):
    ############################### Input
    nr = 301  # number of grid points along radial direction
    # R0 = 1e10 # inner radius of the annulus
    dtheta = n  # 0.35*R0 # d(theta)/d(phi) which should be an integer(winding number) # n=1 is the ground state!!!!! i.e. dtheta=1
    h = 0.1  # step size
    tol = 1e-10
    ...
    return eta1, eta2, j1.imag, f.real
...
for ii, n in enumerate([1, 0.147 * R0, 0.35 * R0]):
    eta1, eta2, j, f = calc_chiral(n, R0)
    ax[0].plot(r, abs(eta1), color=C[ii], label=Lphi[ii])
    ax[0].plot(r, abs(eta2), color=C[ii], linestyle="--", label=Lr[ii])
    ax[1].plot(r, j)
    ax[2].plot(r, f, label="{:.2f}".format((n - 1) / R0))

fs = 15
ax[0].set_ylabel(r"$|\eta|/\eta_0$", fontsize=fs)
ax[0].legend()
ax[0].grid(True)
ax[1].set_ylabel(r"$j/j_0$", fontsize=fs)
ax[1].grid(True)
ax[2].set_ylabel(r"$f/f_0$", fontsize=fs)
ax[2].set_xlabel(r"$(r-R)/\xi$", fontsize=fs)
ax[2].legend()
ax[2].grid(True)
plt.tight_layout()
fig.savefig("chiral.pdf")
