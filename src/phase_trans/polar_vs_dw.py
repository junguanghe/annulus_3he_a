...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 25 11:37:27 2020
...
def F_diff_polar_chiral(D,dtheta):
    ...
    ############################ initial guess
    eta0 = np.array([2*y/D, np.tanh(D/2-np.abs(y))*1j], dtype=complex)         # chiral phase -p+ip|p+ip
    ############################ initial guess
    
    F_polar = -2/3*D*(3*(dtheta)**2-1)**2
    _,eta2,_,F,_,_ = lateral_flow(h,dtheta,eta0)
...
n = 20
dtheta_span = np.linspace(0.3,1/np.sqrt(3),n)
...
np.savez('polar_vs_dw.npz',D_trans=D_trans,dtheta_span=dtheta_span)