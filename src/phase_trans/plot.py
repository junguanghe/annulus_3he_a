...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 25 15:03:22 2020
...
plt.plot(np.concatenate([D_polar_dw[::-1],D_dw_chiral]), np.concatenate([dtheta_polar_dw[::-1],dtheta_dw_chiral]), '-k', lw=2)
plt.plot(D_polar_chiral, dtheta_polar_chiral, '-k', lw=2) # times sqrt(2) to change the y-axis unit into vs/vc, where vc=1/(sqrt(2)xi)
plt.xlim(0,30)
plt.ylim(0,0.58)
plt.text(0.5, .25, 'Polar')
plt.text(10, .05, 'Chiral')
plt.text(10, .35, r'Axial DW')
