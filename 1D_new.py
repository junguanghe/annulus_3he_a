...
def GL(u, h, n, r, BC, Omega):
    ...
    G1 = -eta1 + 0.5*ees*eta1 + 0.25*ee*np.conj(eta1) + (1+3*n**2)*eta1/r**2\
        - 4*1j*n*eta2/r**2 - (de1/r + 2*1j*n*de2/r)
    G2 = -eta2 + 0.5*ees*eta2 + 0.25*ee*np.conj(eta2) + (3+n**2)*eta2/r**2\
        + 4*1j*n*eta1/r**2 - (3*de2/r + 2*1j*n*de1/r)
    G1[1:-1] -= np.diff(eta1, n=2)/h**2
    G2[1:-1] -= 3*np.diff(eta2, n=2)/h**2
    
    G1 -= Omega*(-1j)*(6*1j*n*eta1 + 6*eta2 + 2*r*de2)
    G2 -= Omega*(-1j)*(2*1j*n*eta2 + 2*eta1 + 2*r*de1)
...
def calc_j_f_L(eta1, eta2, dtheta, r, h, Omega):
    ees = eta1*np.conj(eta1) + eta2*np.conj(eta2)
    ee  = eta1**2 + eta2**2
    ...
    f = -ees + 0.25*ees**2 + 0.125*ee*np.conj(ee)
    
    f += - ees*dd1\
    +de1*np.conj(de1) + 3*de2*np.conj(de2) - 2*eta1*np.conj(eta1)*dd1\
    +d1*(eta1*np.conj(de2)+eta2*np.conj(de1)-np.conj(eta1)*de2-np.conj(eta2)*de1) # grad terms in Cartesian basis
    
    f += 2/r*(3*np.conj(eta2)*eta1*d1 - np.conj(eta1)*eta2*d1\
                  + np.conj(eta2)*de2 - np.conj(eta1)*de1).real\
         + 1/r**2*(ees + 2*eta2*np.conj(eta2)) # extra terms besides the terms in Cartesian basis
    
    j1_phi = 3*np.conj(eta1)*eta1*d1 + np.conj(eta2)*eta2*d1 + 4*np.conj(eta1)*eta2/r
    
    j1_r = np.conj(eta2)*de1 + np.conj(eta1)*de2
    
    j1 = j1_phi + j1_r
    
    j2 = 3*np.conj(eta2)*de2 + np.conj(eta1)*de1 + np.conj(eta1)*eta2*d1\
    ...
    return j1, j2, f, F, l, L, L_phi, L_r, Fprime

def plot_results(eta1, eta2, j1, j2, f, r):
    plt.figure()
...
h = 0.1 # step size
R0 = 1e3 # inner radius of the annulus
tol = 1e-9
...
sol = optimize.root(GL, u0, args=(h,dtheta,r,BC,Omega), method='krylov',options={'disp':True, 'xatol':tol})
u = sol.x
...
j1, j2, f, F, l, L, L_phi, L_r, Fprime = calc_j_f_L(eta1, eta2, dtheta, r, h, Omega)
print("average Free Energy density=", Fprime/(V2-V1))
print("Angular momentum L =", L/(V2-V1))
print("Angular momentum L_phi =", L_phi/(V2-V1))
print("Angular momentum L_r =", L_r/(V2-V1))

############################ plot results
plot_results(eta1, eta2, j1, j2, f, r)
