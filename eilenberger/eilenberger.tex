...
Reference Kita's book section 14.1 and Vorontsov 2003,
from the definition of the Green's function,
we have symmetry relations
\begin{align} \label{symmetry_relation}
    \hat{\underline{f}}(\vb{p}, \vb{R}; \epsilon_n)
    = \hat{f}(-\vb{p}, \vb{R}; \epsilon_n)^*
    = -\hat{f}(\vb{p}, \vb{R}; -\epsilon_n)^\dagger \\
    \hat{\underline{g}}(\vb{p}, \vb{R}; \epsilon_n)
    = \hat{g}(-\vb{p}, \vb{R}; \epsilon_n)^*
    = \hat{g}(-\vb{p}, \vb{R}; -\epsilon_n)^T
\end{align}

\section{Current density}
Reference Kita's book, section 14.3.3.
...
\begin{align}
    \vb{j}(\vb{R}) = T\sum_n\frac{1}{V}\sum_p\frac{\vb{p}}{m^*}
    \Tr{\hat{G}(\vb{R}, \vb{p};i\epsilon_n)}
\end{align}
In 2d case, we have
\begin{align}
    \frac{1}{V}\sum_p = \int\frac{\dd[2]p}{(2\pi\hbar)^2}
    =\iint \frac{p\dd\theta_p\dd p}{(2\pi\hbar)^2}
    = \int N_f\dd\xi_p\int\frac{\dd\theta_p}{2\pi}
    \qq{where} N_f = \frac{m^*}{2\pi\hbar^2}
\end{align}
The particle density is derived by dividing the total phase space volume $\pi k_f^2$
by the volume of the unit cell $(2\pi/L)^2$, times 2 for spin.
\begin{align}
    N = 2\times\frac{\pi p_f^2/\hbar^2}{(2\pi/L)^2} = \frac{p_f^2}{2\pi\hbar^2}V
    \rightarrow n = \frac{N}{V} = \frac{p_f^2}{2\pi\hbar^2} = N_fp_fv_f
\end{align}
In 3d case, we have
\begin{align}
    \frac{1}{V}\sum_p = \int\frac{\dd[3]p}{(2\pi\hbar)^3}
    =\int\frac{m^*p\dd\xi_p}{2\pi^2\hbar^3} \frac{\dd\Omega_p}{4\pi}
    \approx \int N_f\dd\xi_p\int\frac{\dd\Omega_p}{4\pi}
    \qq{where} N_f = \frac{m^*p_f}{2\pi^2\hbar^3}
\end{align}
and
\begin{align}
    N = 2\times\frac{\frac{4}{3}\pi p_f^3/\hbar^3}{(2\pi/L)^3} = \frac{p_f^3}{3\pi^2\hbar^3}V
    \rightarrow n = \frac{N}{V} = \frac{2}{3}N_fp_fv_f
\end{align}
Do the $\xi_p$ integration, we get
...
\begin{align}
    \vb{j}_m(\vb{R}) \equiv m^*\vb{j}(\vb{R}) = \frac{N_fv_fp_f}{4}\hbar \times
    \frac{4T}{\xi(T)|\Delta(T)|}\sum_n\ev{\vu{p} g(\vb{R}, \vb{p}, \epsilon_n)}_{p}
\end{align}
where $\xi(T) = \hbar v_f/2|\Delta(T)|$ is the coherence length, and $n = N_fp_fv_f$ is the particle density.
The ground state angular momentum in a cylinder is
\begin{align} \label{L_eq}
    \vb{L} & = \int_0^R \dd[2]r \vb{r} \times \vb{j}_m(\vb{r})            \\
           & = 2\pi\vu{z}\int_0^R \dd{r} r^2 j_m(r)                       \\
           & \approx 2\pi R^2\vu{z}\int_{R-\xi}^R \dd{r} j_m(r)           \\
           & \approx 2V\vu{z} \int_0^R \dd{r} j_m(r)                      \\
           & = \frac{N}{2}\hbar \vu{z} \times\frac{4T}{\xi(T)|\Delta(T)|}
    \int_0^R \dd{r} \sum_n\ev{p_y g(\vb{r}, \vb{p}, \epsilon_n)}_{p}
\end{align}

\section{Constant Gap}
For constant gap, reference Sauls 2011, we have
\begin{align}
    g(x, \vb{p}, \epsilon_n) = -\frac{\pi i\epsilon_n}{\lambda}
    - \frac{\pi\Delta_1}{\lambda}
    \frac{i\epsilon_n\Delta_1 + \lambda\Delta_2}{\Delta_2^2 + \epsilon_n^2}
    e^{-\frac{\lambda}{\Delta_1}\frac{x}{\xi}}
\end{align}
where here we denote $\Delta_{1,2} \equiv \Delta p_{x,y}$,
and $\lambda = \sqrt{\Delta^2 + \epsilon_n^2}$.
The $i\epsilon_n$ term doesn't contribute to the angular momentum,
as its Matsubara sum gives zero. After we integrate over the radial coordinate, we get
\begin{align}
    L = \frac{N}{2}\hbar \times \frac{4T}{\Delta}\sum_n
    \ev{p_y \pi\frac{\Delta_1^2}{\lambda} \frac{\Delta_2}{\Delta_2^2 + \epsilon_n^2}}_{p}
\end{align}
According to ChatGPT and Gemini, the angle integral gives
\begin{align}
    \int_0^{2\pi}\dd\theta_p \frac{p_x^2 p_y^2}{p_y^2 + a^2}
    = \pi \left(\sqrt{a^2 + 1} - |a|\right)^2
\end{align}
and we have
\begin{align}
    L=\frac{N}{2}\hbar \times
    4\pi^2T\sum_n\frac{\left(\lambda-|\epsilon_n|\right)^2}{\Delta^2\lambda}
\end{align}
According to ChatGPT and Gemini, the Matsubara sum gives
\begin{align}
    L=\frac{N}{2}\hbar \times 2\pi
\end{align}
we are clearly missing a factor of $2\pi$ here.
...
\begin{align} \label{riccati_param}
    \widehat{g}
    = -i\pi\widehat{N}
    \begin{pmatrix}
...
\end{align}
We can also write the anomalous Green's function as corresponding components to $\Delta_{1,2}$
\begin{align} \label{f_decomp}
...
\begin{align}
    ...
    {\sum_n \frac{\pi}{\sqrt{|\Delta|^2 + \epsilon_n^2}}}
\end{align}
We only need to sum over the positive Matsubara frequencies and take real part because
if we substitute Eq. (\ref{f_decomp},\ref{f_decomp_2}) into Eq. (\ref{symmetry_relation}), we get
\begin{align}
    f_1(\epsilon_n) - if_2(\epsilon_n) = f_1^*(-\epsilon_n) - if_2^*(-\epsilon_n)
\end{align}
Since $f_{1,2}$ are seperate components in the (particle-hole $\otimes$ spin) space, we have
\begin{align}
    f_{1,2}(\epsilon_n) = f_{1,2}^*(-\epsilon_n)
\end{align}
Similarly we also have
\begin{align}
    -f_{1,2}(-\vb{p}, \vb{R}, \epsilon_n) = f_{1,2}(\vb{p}, \vb{R}, -\epsilon_n)
\end{align}
and we also only need to average the angular part over a half circle, because
\begin{align}
      & \int_{0}^{2\pi}\frac{\dd\theta_{p'}}{2\pi}  \vb{p}_{x,y}\vdot\vb{p}' f_{1,2}(\vb{p}', \vb{R}, \epsilon_n)     \\
    = & \left(\int_{-\pi/2}^{\pi/2} + \int_{\pi/2}^{3\pi/2}\right)\frac{\dd\theta_{p'}}{2\pi}\vb{p}_{x,y}\vdot\vb{p}'
    f_{1,2}(\vb{p}', \vb{R}, \epsilon_n)                                                                              \\
    = & \int_{-\pi/2}^{\pi/2}\frac{\dd\theta_{p'} }{2\pi}\vb{p}_{x,y}\vdot\vb{p}'
    \bigg[f_{1,2}(\vb{p}', \vb{R}, \epsilon_n) - f_{1,2}(-\vb{p}', \vb{R}, \epsilon_n)\bigg]                          \\
    = & \int_{-\pi/2}^{\pi/2}\frac{\dd\theta_{p'} }{2\pi}\vb{p}_{x,y}\vdot\vb{p}'
    \bigg[f_{1,2}(\vb{p}', \vb{R}, \epsilon_n) + f_{1,2}(\vb{p}', \vb{R}, -\epsilon_n)\bigg]                          \\
    = & \int_{-\pi/2}^{\pi/2}\frac{\dd\theta_{p'} }{2\pi}\vb{p}_{x,y}\vdot\vb{p}'
    \bigg[f_{1,2}(\vb{p}', \vb{R}, \epsilon_n) + f_{1,2}^*(\vb{p}', \vb{R}, \epsilon_n)\bigg]                         \\
    = & \int_{-\pi/2}^{\pi/2}\frac{\dd\theta_{p'} }{\pi}\vb{p}_{x,y}\vdot\vb{p}'
    \Re{f_{1,2}(\vb{p}', \vb{R}, \epsilon_n)}
\end{align}
...
\begin{align}
    \delta_{1,2}(x) =
    \frac{2\sum_{n\geq0} \ev{\vb{p}_{x,y}\vdot\vb{p}'\Re{f_{1,2}(x, \vb{p}', n)}}_{p'}}
    {\sum_{n\geq0} \frac{\pi}{\sqrt{1 + e_n^2/\delta(t)^2}}}
\end{align}
where $t = T/T_c$, $\delta_{1,2}(x) = \Delta_{1,2}(x)/|\Delta(T)|$,
$e_n = \epsilon_n/T_c$ and $\delta(t) = |\Delta(T)|/T_c$.
The Ricatti equation is
\begin{align}
    ip_x\partial_x a(\vb{p},x) + \frac{ie_n}{\delta(t)}a(\vb{p},x)
    + \frac{\delta^*(\vb{p},x)}{2} a^2(\vb{p},x) + \frac{\delta(\vb{p},x)}{2} = 0 \\
    ip_x\partial_x \underline{a}(\vb{p},x) - \frac{ie_n}{\delta(t)} \underline{a}(\vb{p},x)
...
We can get $g$ and $f_{1,2}$ from Eq. (\ref{f_decomp_3}) and Eq. (\ref{riccati_param}),
\begin{align}
    g = -i\pi\frac{1+a\underline{a}}{1-a\underline{a}}    \\
    f_1 = -i\pi\frac{a + \underline{a}}{1-a\underline{a}} \\
    f_2 = -\pi\frac{a - \underline{a}}{1-a\underline{a}}
\end{align}
