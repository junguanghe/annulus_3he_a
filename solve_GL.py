...
def calc_j_f_L(eta1, eta2, dtheta, r, h):
    ...
    f = -ees + 0.25*ees**2 + 0.125*ee*np.conj(ee) - ees*dd1\
    +de1*np.conj(de1) + 3*de2*np.conj(de2) - 2*eta1*np.conj(eta1)*dd1\
    +d1*(eta1*np.conj(de2)+eta2*np.conj(de1)-np.conj(eta1)*de2-np.conj(eta2)*de1)
    ...
    j1 = 3*np.conj(eta1)*eta1*d1 + np.conj(eta2)*eta2*d1\
    +np.conj(eta2)*de1 + np.conj(eta1)*de2 + 4*np.conj(eta1)*eta2/r
